...
function InvitesReceived() {
  ...
  const booking = location.state?.booking; // State se booking data retrieve kar rahe hain
  // const [showCounterOfferPopup, setShowCounterOfferPopup] = useState(false);
  console.log("Location State:", location.state); // Log the entire state object
  console.log("Booking Data:", booking); // Log the booking data specifically

// Added state variables for counter offer input
  const [showCounterOfferPopup, setShowCounterOfferPopup] = useState(false);
  ...
  // Use jobId from booking, fallback to id if not available
  const jobId = booking.jobId || booking.job?._id || id;
  if (!jobId) {
    console.error("Job ID is undefined. Check booking structure:", booking);
  ...
  const payload = {
    ...
    offer: {
      ...
      type: rateType,
      eventFee: booking.paymentInfo?.eventFee || `${booking.paymentInfo?.currency || 'USD'} 0`,
      bookingFee: booking.paymentInfo?.bookingFee || `${booking.paymentInfo?.currency || 'USD'} ${Math.round(parseFloat((booking.paymentInfo?.eventFee || '0').replace(`${booking.paymentInfo?.currency || ''}`, '')) * 0.10)}`,
      paidToHostess: booking.paymentInfo?.paidToHostess || "N/A",
  ...
  try {
    console.log("Sending payload to URL:", apiUrl, payload);
    ...
    console.error("Error submitting counter offer:", error.response ? error.response.data : error.message, "URL:", apiUrl);
  ...
  <option value="perDay">Per Day</option>
                  ...
                  <div className="inline-flex justify-start items-center gap-2">
                    <div className="justify-start text-[#656565] text-xs font-normal font-['Inter'] leading-none">
                      {/* 10% Event fee: $10 */}
                      {/* 10% Event fee: {booking.paymentInfo?.eventFee ? `${booking.paymentInfo?.job.currency} ${Math.round((parseFloat(booking.paymentInfo.eventFee.replace(`${booking.paymentInfo?.job.currency}`, '')) * 0.10))}` : "N/A"} */}
                       {/* 10% Event fee: {booking.paymentInfo?.eventFee ? `${booking.paymentInfo.currency} ${Math.round((parseFloat(booking.paymentInfo.eventFee.replace(`${booking.paymentInfo.currency}`, '')) * 0.10))}` : "N/A"} */}
                       10% Event Fee: {booking.paymentInfo?.eventFee ? `${booking.paymentInfo.currency} ${Math.round(parseFloat(booking.paymentInfo.eventFee.replace(`${booking.paymentInfo.currency}`, '')) * 0.10)}` : "N/A"}
                    </div>
                    <div className="w-3.5 self-stretch origin-top-left rotate-90 outline outline-1 outline-offset-[-0.50px] outline-[#ECECEC]"></div>
                    <div className="justify-start text-[#656565] text-xs font-normal font-['Inter'] leading-none">
                      {/* You’ll Receive $90/h */}
                      {/* You’ll Receive {booking.paymentInfo?.eventFee ? `${booking.paymentInfo?.job.currency} ${Math.round(parseFloat(booking.paymentInfo.eventFee.replace(`${booking.paymentInfo?.job.currency}`, '')) * 0.90 / (booking.duration ? parseInt(booking.duration) : 1))}/h` : "N/A"} */}
                       {/* You’ll Receive {booking.paymentInfo?.eventFee ? `${booking.paymentInfo.currency} ${Math.round(parseFloat(booking.paymentInfo.eventFee.replace(`${booking.paymentInfo.currency}`, '')) * 0.90 / (booking.duration ? parseInt(booking.duration) : 1))}/h` : "N/A"} */}
                       You’ll Receive: {booking.paymentInfo?.eventFee ? `${booking.paymentInfo.currency} ${Math.round(parseFloat(booking.paymentInfo.eventFee.replace(`${booking.paymentInfo.currency}`, '')) * 0.90 / (booking.duration ? parseInt(booking.duration) : 1))}/h` : "N/A"}
                    </div>
                  </div>
                </div>
