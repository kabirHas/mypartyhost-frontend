...
function FindJobs() {
  ...
  try {
    ...
    const response = await axios.post(
      `https://mypartyhost.onrender.com/api/jobs/${jobId}/saved`,
      {},
  ...
  const handleNearMe = () => {
         ...
         navigator.geolocation.getCurrentPosition(
           (position) => {
             ...
             const nearby = jobs
               ...
               .map(job => ({
                 ...job,
                 distance: calculateDistance(latitude, longitude, job.lat, job.lng)
               }))
    ...
    try {
      ...
      if (geoResponse.data && geoResponse.data[0]) {
        ...
        } else if (display_name.toLowerCase().includes("india") || locationSearch.toLowerCase() === "india") {
          zoomLevel = 3;
          locationType = "country";
        }
        ...
        if (locationType === "country") {
          const searchedCountry = address.country ? address.country.toLowerCase() : searchLower;
          ...
          console.log("Country search results:", locationFiltered.map((job) => ({
            id: job.id,
            location: job.location,
            country: job.country,
          }))); // fix-5: Log country search results
        } else if (locationType === "city") {
          const searchedCity = address.city ? address.city.toLowerCase() : searchLower;
          locationFiltered = jobs
            ...
            .map((job) => ({
              ...job,
              distance: calculateDistance(parseFloat(lat), parseFloat(lon), job.lat, job.lng),
            }))
            .filter((job) => job.distance <= 50)
            .sort((a, b) => a.distance - b.distance);
          console.log("City search results:", locationFiltered.map((job) => ({
            id: job.id,
            location: job.location,
            city: job.city,
          ...
          const searchedSuburb = address.suburb ? address.suburb.toLowerCase() : searchLower;
          locationFiltered = jobs.filter(
            (job) =>
              (job.suburb && job.suburb.toLowerCase().includes(searchedSuburb)) ||
              job.location.toLowerCase().includes(searchLower) // fix-4: Fallback to location string
          );
          console.log("Suburb search results:", locationFiltered.map((job) => ({
            id: job.id,
            location: job.location,
            suburb: job.suburb,
            lat: job.lat,
        ...
        setShowMap(true);
        console.log(`Set nearbyJobs for ${locationType} search:`, locationFiltered.length); // fix-6: Log number of filtered jobs
      } else {
  ...
  try {
    ...
    const savedJobIds = (response.data.savedJobs || response.data).map(job => job._id);
...
useEffect(() => {
  const fetchJobs = async () => {
    try {
      ...
      console.log('Filtered jobs:', activeJobs.map(job => ({
        id: job._id,
        jobDate: job.jobDate,
      ...
      const formatJob = (job, lat = null, lng = null) => ({
        ...
        organiser: {
          ...
          profileImage: job.organiser.profileImage || "https://placehold.co/48x48",
      ...
      for (const job of activeJobs) {
        try {
          let geoResponse = await axios.get(
            ...
            {
              params: {
                q: `${job.suburb || ''}${job.suburb ? ',' : ''} ${job.city || ''}${job.city ? ',' : ''} ${job.country || ''}`.trim().replace(/,+/g, ','),
                format: "json",
                limit: 1,
              },
              headers: {
          ...
          if (geoResponse.data && geoResponse.data[0]) {
            updatedJobs.push(formatJob(job, parseFloat(geoResponse.data[0].lat), parseFloat(geoResponse.data[0].lon)));
            console.log(`Geocoding success for ${job.suburb || ''}, ${job.city || ''}, ${job.country || ''}:`, {
              lat: parseFloat(geoResponse.data[0].lat), 
              lng: parseFloat(geoResponse.data[0].lon), 
              jobId: job._id
            });
          } else {
            console.warn(`Geocoding failed for ${job.suburb || ''}, ${job.city || ''}, ${job.country || ''}: No results, retrying with city and country`); // comment-4: Improved logging for retry
            // Retry with city and country only
            geoResponse = await axios.get(
              `https://nominatim.openstreetmap.org/search`,
              {
                params: {
                  q: `${job.city || ''}${job.city ? ',' : ''} ${job.country || ''}`.trim().replace(/,+/g, ','),
                  format: "json",
                  limit: 1,
                },
                headers: {
            ...
            if (geoResponse.data && geoResponse.data[0]) {
              updatedJobs.push(formatJob(job, parseFloat(geoResponse.data[0].lat), parseFloat(geoResponse.data[0].lon)));
              console.log(`Geocoding success on retry for ${job.city || ''}, ${job.country || ''}:`, {
                lat: parseFloat(geoResponse.data[0].lat),
                lng: parseFloat(geoResponse.data[0].lon),
                jobId: job._id 
              });
            } else {
              console.warn(`Geocoding failed for ${job.city || ''}, ${job.country || ''}: No results`); // comment-6: Log final geocoding failure
              updatedJobs.push(formatJob(job));
            }
          }
        } catch (error) {
          console.error(`Geocoding error for ${job.suburb || ''}, ${job.city || ''}, ${job.country || ''}:`, error.message); // comment-7: Enhanced error logging
          updatedJobs.push(formatJob(job));
        }
      }
      ...
      const userData = localStorage.getItem("token") ? JSON.parse(localStorage.getItem("userData")) : null;
      if (userData) {
        let userQuery = `${userData.suburb || ''}${userData.suburb ? ',' : ''} ${userData.city || ''}${userData.city ? ',' : ''} ${userData.state || ''}`.trim().replace(/,+/g, ',');
        let geocoded = false;
        if (userQuery) {
          try {
            const geoResponse = await axios.get(
      ...
      setGeocodedJobs(updatedJobs);
      setJobs(updatedJobs);
      console.log('Jobs set to state:', updatedJobs.map(job => ({
        id: job.id,
        lat: job.lat,
        lng: job.lng,
        location: job.location
  ...
  return (
    <div className="self-stretch inline-flex flex-col justify-start items-start gap-12 font-['Inter']">
      <div className="w-full  flex flex-col justify-start items-start gap-12">
          ...
          <div className="self-stretch flex flex-col justify-start items-start gap-4">
            ...
            {!showMap ? (
              <>
                ...
                {(nearbyJobs || filteredJobs).slice((currentPage - 1) * jobsPerPage, currentPage * jobsPerPage).map((job) => (
                  <div
                    key={job.id}
                      ...
                      {/* <div className="px-6 py-3 rounded-lg outline outline-1 outline-offset-[-1px] outline-[#E61E4D] flex justify-center items-center gap-2 overflow-hidden"> */}
                        <div
                          className={`px-6 py-3 rounded-lg outline outline-1 outline-offset-[-1px] flex justify-center items-center gap-2 overflow-hidden ${
                            savedJobs.includes(job.id) ? "bg-[#FFF1F2] outline-[#E61E4D] text-[#E61E4D]" : "outline-[#E61E4D] text-[#E61E4D]"
                          }`}
                        ...
                        <div className="w-6 h-6 relative">
                           <i className={`ri-heart-3-${savedJobs.includes(job.id) ? "fill" : "line"} text-[#E61E4D]`}></i>
                ...
                <div className="self-stretch flex flex-col justify-center items-end gap-2.5">
                  <div className="inline-flex justify-start items-center gap-2">
                    ...
                    > */}
                    <div className={`px-3 py-2 rounded-lg flex justify-center items-center gap-2 ${currentPage === 1 ? "opacity-50 cursor-not-allowed" :  "cursor-pointer"}`} onClick={() => currentPage > 1 && setCurrentPage(currentPage - 1)}
                      >
                    ...
                    <div className="flex justify-start items-center gap-2">
                      {Array.from({ length: Math.ceil(filteredJobs.length / jobsPerPage) }, (_, i) => i + 1).map((n) => (
                          <button
                            key={n}
                            onClick={() => setCurrentPage(n)}
                            className={`px-3 py-1 rounded-md ${n === currentPage ? "bg-[#FFCCD3] text-[#292929]" : "text-gray-600"}`}
                          >
                            {n}
                          </button>
                        ))}
                    ...
                    <div
                          className={`px-3 py-2 rounded-lg flex justify-center items-center gap-2 ${currentPage === Math.ceil(filteredJobs.length / jobsPerPage) ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
                          onClick={() => currentPage < Math.ceil(filteredJobs.length / jobsPerPage) && setCurrentPage(currentPage + 1)}
                        >
                ...
                <div className="self-stretch inline-flex justify-between items-center">
                  ...
                  <div className="px-3 py-1 rounded-lg outline outline-1 outline-offset-[-1px] outline-[#656565] inline-flex justify-center items-center gap-2">
                    ...
                    <input type="text"
                      ...
                      placeholder="Search location, suburb" className="bg-transparent focus:outline-none text-[#656565] text-sm font-normal font-['Inter'] leading-tight">
                    </input>
                  </div>
        ...
        console.log('Jobs displayed on map:', jobsToDisplay.filter(job => job.lat && job.lng).map(job => ({
          id: job.id,
          lat: job.lat,
          lng: job.lng,
          location: job.location
      ...
      <div className="w-full text-center py-4">
        <p className="text-[#3D3D3D] text-base font-normal font-['Inter'] leading-snug">
          No jobs available in {searchedLocationName || "this location"}.
        </p>
      </div>
    ) : (
      <>
                        ...
                        <div className="flex justify-center items-center gap-6 mt-2">
                          ...
                          <button
                            className={`px-4 py-2 border-2 font-semibold border-[#E61E4D] text-[#E61E4D] text-sm rounded-lg flex items-center gap-1 ${
                              savedJobs.includes(job.id) ? "bg-[#FFF1F2]" : ""
                            }`}
                            onClick={() => handleSaveJob(job.id)}
                          >
                            {savedJobs.includes(job.id) ? "Saved" : "Save Job"}
                            <i className={`ri-heart-3-${savedJobs.includes(job.id) ? "fill" : "line"}`}></i>
                          </button>
                          <button
